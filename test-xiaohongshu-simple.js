...
if (dataMatches && dataMatches.length > 0) {
  ...
  for (let i = 0; i < Math.min(10, dataMatches.length); i++) {
    const match = dataMatches[i];
    const valueMatch = match.match(/"(?:desc|title|content)":"([^"]*)"/);  
    if (valueMatch && valueMatch[1]) {
      const text = valueMatch[1].trim();
      if (text.length > 10 && !text.match(/^\d+$/) && !text.match(/javascript|function/i)) {
        console.log(`  [${i+1}] 长度:${text.length}, 内容: ${text.substring(0, 80)}`);
        if (!foundContent) {
          console.log(`  ✓ 第一个有效内容: ${text}`);
          foundContent = true;
        }
      }
    }
